 ...
 */
class VideoSegmentSelectorPlugin implements JsPsychPlugin<Info> {
  static info = info;
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    // --------------------- FOR ALEX: duplicating code here to see if two sliders can be added ------------------------------
    ...
    if (trial.require_movement) {
      ...
      // FOR ALEX: I think copying this piece of code and using it somewhere else to change video_element.currentTime might give us the behavior we want (frame skip on slider change)
      display_element // from the display_element object that has all elements
        .querySelector("#jspsych-video-slider-response-response") // query the slider (the large #)
        .addEventListener("change", enable_button); // and add an EventListener to that slider, that says "on event 'change', call the function 'enable_button' "
    }


    // FOR ALEX: This is what we need, probably
    display_element
        .querySelector("#jspsych-video-slider-response-response")
        .addEventListener("change", (e) => {console.log(e)
                                            video_element.currentTime=e.target.value; // we need to skip to the right second
                                            });
...
export default VideoSegmentSelectorPlugin;